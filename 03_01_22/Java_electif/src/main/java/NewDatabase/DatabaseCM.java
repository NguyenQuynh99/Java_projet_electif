...
public class DatabaseCM {
    ...
    public String[][] listCM(){
        String[][] listCM = new String[101][11];
        ...
        String[] groupe1 = cm.createGroupe1();
        String[] groupe2 = cm.createGroupe2();
        for (int i=0; i<this.etudiants.size(); i++){
            listCM[i][0]=listEtudiant[i];
            for (int j=0; j<10; j++){
                double cout = coutAlea(new Random());
                if (j<5){
                    listCM[0][j+1]=groupe1[j];
                    listCM[i+1][j+1] = " " + cout;
                }
                else{
                    int nm = j-5;
                    listCM[0][j+1]=groupe2[nm];
                    listCM[i+1][j+1] = " " + cout;
                }
            }
            //listCM[i][1]=groupe1[i];
            //listCM[i][2]=groupe2[i];
        }
        return listCM;
    }
    ...
    public static void main(String[] args){
        ...
        String[][] listCM = databaseCM.listCM();
